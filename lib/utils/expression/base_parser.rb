...
module Expression
class BaseParser
  def initialize(exp, field, prd= ["AND", "OR"])
  ...
  attr_accessor :char_array, :exp_array, :stack, :field_name, :precedence
  ...
  def parse(sub, op)
    ...
    while start < sub.count
      if  sub[start] == op
        prev_val = dsl.pop
        dsl << send("#{op.downcase}_parser",prev_val, sub[start + 1])
        start += 1
      else
        dsl << sub[start]
      end
...
a = ExpressionToDsl.new('(Java AND Spring) OR (Python AND Django) OR (Ruby AND ("Ruby on Rails" OR ROR))')
